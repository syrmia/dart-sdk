...
#ifndef RUNTIME_VM_ELFGEN_H_
#define RUNTIME_VM_ELFGEN_H_
...
void ElfGen::AddCodeRegion(const char* name, uword pc, intptr_t size) {
  ...
  char end_name[256];
  Utils::SNPrint(end_name, sizeof(end_name), "%s_end", name);
...
#error Unknown architecture.
  ...
  WriteShort(&header_, kEM_AARCH64);  // e_machine
#elif defined(TARGET_ARCH_MIPS)
  WriteShort(&header_, kEM_MIPS);  // e_machine
#elif defined(TARGET_ARCH_RISCV32) || defined(TARGET_ARCH_RISCV64)
  WriteShort(&header_, kEM_RISCV);  // e_machine
#else
#error Unknown architecture.
#endif
