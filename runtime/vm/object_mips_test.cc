...
void GenerateIncrement(compiler::Assembler* assembler) {
  __ Push(ZR);
  __ lw(TMP, compiler::Address(SP, 0));
  __ addiu(TMP, TMP, compiler::Immediate(1));
  __ sw(TMP, compiler::Address(SP, 0));
  __ lw(TMP, compiler::Address(SP, 0));
  __ addiu(TMP, TMP, compiler::Immediate(1));
  __ Pop(V0);
  __ mov(V0, TMP);
...
void GenerateEmbedStringInCode(compiler::Assembler* assembler, const char* str) {
