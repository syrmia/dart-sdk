...
ASSEMBLER_TEST_GENERATE(Lb, assembler) {
  __ addiu(SP, SP, Immediate(-target::kWordSize * 30));
...
ASSEMBLER_TEST_GENERATE(StoreHalfWordUnaligned, assembler) {
  ...
  __ StoreHalfWordUnaligned(A1, A0, TMP);
  __ mov(V0, A1);
  __ jr(RA);
}
...
ASSEMBLER_TEST_GENERATE(StoreIntoObject, assembler) {
  ...
  __ sw(CODE_REG, Address(SP, 2 * target::kWordSize));
  __ sw(THR, Address(SP, 1 * target::kWordSize));
  __ sw(RA, Address(SP, 0 * target::kWordSize));
  __ mov(THR, A2);
  __ StoreIntoObject(A1, FieldAddress(A1, GrowableObjectArray::data_offset()),
                     A0);
  __ lw(RA, Address(SP, 0 * target::kWordSize));
  __ lw(THR, Address(SP, 1 * target::kWordSize));
  __ lw(CODE_REG, Address(SP, 2 * target::kWordSize));
  __ addiu(SP, SP, Immediate(3 * target::kWordSize));
  __ Ret();
}
